...
class VCFF_Single_File_Field_Upload extends VCFF_Helper {
    ...
    public function _Build_Instances() {
    ...
    public function _AJAX_Remove() {
        // Build the required instances
        $this->_Build_Instances();
        // Retrieve the form instance
        $form_instance = $this->form_instance;
        // Retrieve the field instance
        $field_instance = $this->field_instance;
        // Retrieve the upload path
        $tmp_upload_path = $this->_Get_Tmp_Upload_Path();
    ...
    public function _AJAX_Upload() {
        ...
        $mimetypes = $this->_Get_Mime_Types();  
        ...
        // Retrieve the upload path
        $upload_path = $this->_Get_Upload_Path();
        // Retrieve the upload path
        $tmp_upload_path = $this->_Get_Tmp_Upload_Path();
        ...
        while($search == true) {
            ...
            $possible_filename = $this->_Prefix().'_'.$raw_filename.'.'.$extension; 
    ...
    public function Confirm_Upload($data) {
        // Retrieve the filename
        $filename = sanitize_file_name($data['name']);
        // Don't trust the provided location information
        $upload_path = $this->_Get_Upload_Path();
        // Retrieve the upload path
        $tmp_upload_path = $this->_Get_Tmp_Upload_Path();
        // If the file doesn't exist, return out
        if (!file_exists($tmp_upload_path.'/'.$filename)) { return; }
        // Move the file to the new location
        rename($tmp_upload_path.'/'.$filename,$upload_path.'/'.$filename);
    }
    ...
    public function _Get_Tmp_Upload_Path() {
        // Retrieve the upload base path
        $tmp_path = vcff_get_setting_value('field_upload_temp_dir');
        // Retrieve the tmp prefix
        $tmp_prefix = get_option('vcff_tmp_upload_prefix');
        // If no prefix currently selected
        if (!$tmp_prefix) {
            // Generate a new prefix
            $tmp_prefix = $this->_Prefix();
            // Set the tmp folder prefix
            add_option('vcff_tmp_upload_prefix',$tmp_prefix);
        }
        // If no base path then
        if (!$tmp_path) {
            // Retrieve the wordpress upload dir data
            $wp_upload_dir = wp_upload_dir();
            // Use the default vcff upload path
            $tmp_path = $wp_upload_dir['basedir'].'/vcff/'.$tmp_prefix.'_tmp';
        }
    ...
    public function _Get_Upload_Path() {
        // Retrieve the upload base path
        $base_path = vcff_get_setting_value('field_upload_dir');
        // If no base path then
        if (!$base_path) {
            // Retrieve the wordpress upload dir data
            $wp_upload_dir = wp_upload_dir();
            // Use the default vcff upload path
            $base_path = $wp_upload_dir['basedir'].'/vcff';
        }
        // If the base dir does not exist
        if (!is_dir($base_path)) { mkdir($base_path); }
        // Retrieve the folder structure
        $folder_structure = vcff_get_setting_value('field_upload_structure');
        // If we are using the YYMMDD structure
        if ($folder_structure == 'YYMMDD') {
            // Populate the upload path
            $upload_path = $this->_Upload_Path_YYMMDD($base_path);
        } // Otherwise, If we are using the YYMM structure
        elseif ($folder_structure == 'YYMM') {
            // Populate the upload path
            $upload_path = $this->_Upload_Path_YYMM($base_path);
        } // Otherwise, If we are using the YY structure
        elseif ($folder_structure == 'YY') {
            // Populate the upload path
            $upload_path = $this->_Upload_Path_YY($base_path);
        } // Otherwise, If we are using no structure
        else { $upload_path = $base_path; }
        // Return the upload path
        return $upload_path;
    }
    
    protected function _Upload_Path_YYMMDD($base_path) {
        // Build the base path
        $base_path .= '/'.date('Y');
        // If the required sub dir does not exist
        if (!is_dir($base_path)) { mkdir($base_path); }
        // Build the base path
        $base_path .= '/'.date('m');
        // If the required sub dir does not exist
    ...
    protected function _Upload_Path_YYMM($base_path) {
        // If the base dir does not exist
        if (!is_dir($base_path)) { mkdir($base_path); }
        // Build the base path
        $base_path .= '/'.date('Y');
    ...
    protected function _Upload_Path_YY($base_path) {
        // If the base dir does not exist
        if (!is_dir($base_path)) { mkdir($base_path); }
        // Build the base path
        $base_path .= '/'.date('Y');
    ...
    public function _Get_Mime_Types() {
